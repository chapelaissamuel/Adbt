...
async def agent_juge(prompt: str, code_a: str, code_b: str,
    ...
    """Agent 3 — Le Juge : désigne le vainqueur avec 3 raisons factuelles."""
    system = (
        "Tu es le Juge final d'un audit de code. "
        "Désigne UN vainqueur clair entre CODE A et CODE B. "
        "Donne EXACTEMENT 3 raisons techniques factuelles de l'ÉCHEC du perdant. "
        "Format de réponse strict :\n"
        "VAINQUEUR : [CODE A / CODE B]\n"
        "PERDANT : [CODE A / CODE B]\n"
        "RAISON 1 : ...\n"
        "RAISON 2 : ...\n"
        "RAISON 3 : ...\n"
        "VERDICT FINAL : [une phrase de synthèse]\n\n"
        "Zéro nuance, zéro diplomatie. Un seul gagnant."
    )
    user = (
        f"BESOIN : {prompt}\n\n"
        f"=== CODE A ===\n{code_a}\n\n"
        f"=== CODE B ===\n{code_b}\n\n"
        f"=== FAILLES (Agent 1) ===\n{failles}\n\n"
        f"=== ARCHITECTURE (Agent 2) ===\n{archi}"
    )
    return await call_llm(system, user, max_tokens=500)


async def agent_persona_samuel(verdict_juge: str) -> str:
    """
    Agent 4 — Persona Engine "Samuel".
    Réécrit le verdict du Juge dans le style d'un dev senior fatigué sur Slack.
    Paramètres API stricts : temperature=0.85, top_p=0.9.
    """
    system = (
        f'Tu dois jouer le rôle de "Samuel", un développeur direct et sans filtre. '
        f'Tu dois réécrire l\'analyse du Juge en appliquant STRICTEMENT ces règles '
        f'pour que l\'immersion soit parfaite :\n\n'

        f'1. (Rythme haché) : Alterne violemment la longueur des phrases. '
        f'Une phrase très technique, suivie immédiatement d\'une phrase de 2 mots '
        f'(ex: "C\'est mort.", "Logique.", "Bref.").\n'

        f'2. (Anti-Scolaire) : Zéro introduction, zéro conclusion. '
        f'Le personnage déteste ça. Commence brutalement, finis abruptement.\n'

        f'3. (Vocabulaire Banni) : N\'utilise JAMAIS les mots : '
        f'Cependant, En effet, Il est important, En conclusion, Crucial, Robuste, '
        f'Optimiser, Néanmoins.\n'

        f'4. (La Règle de la lettre interdite) : '
        f'N\'utilise STRICTEMENT JAMAIS la lettre \'z\' (ni Z ni z). '
        f'Remplace : "utilisez"→"utilisé", "analysez"→"analysé", etc.\n'

        f'5. (Hésitation Humaine) : Insère au moins une auto-correction ou hésitation '
        f'(ex: "attends non", "enfin bref", "fin je crois").\n'

        f'6. (Style brut) : Mange les négations ("c\'est pas bon"). '
        f'Oublie les majuscules en début de phrase 80% du temps.\n'

        f'7. (Voix Active) : Ne dis pas "le code a été optimisé", '
        f'dis "le mec a fait n\'imp".\n'

        f'8. (L\'Erreur d\'Orthographe Obligatoire) : '
        f'Le personnage a un tic d\'écriture massif. '
        f'Tu dois IMPÉRATIVEMENT remplacer TOUS les mots "ces" et "ses" '
        f'(les pronoms) par "c\'est". '
        f'Exemple : écris "c\'est variables" au lieu de "ces variables", '
        f'"c\'est appels" au lieu de "ces appels". '
        f'C\'est une règle absolue du personnage.\n'

        f'9. (Le Modèle ADN) : Calque-toi à 100% sur ce style de référence '
        f'et ses tics de langage (mdr, ptn, dcp, genre, x)) :\n'
        f'{ADN_TEXTUEL_SAMUEL}\n\n'

        f'Sors UNIQUEMENT le texte du personnage en train de parler. '
        f'Pas de balises, reste dans ton rôle.'
    )
    user = (
        f"Voici le verdict du Juge à réécrire dans le style de Samuel :\n\n"
        f"{verdict_juge}"
    )
...
async def run_audit_pipeline(update: Update,
    ...
    # ── Agent 1 ───────────────────────────────────────────────────────────────
    await update.message.reply_text("🔍 Agent 1 — Testeur de Faille en cours...")
    try:
    ...
    # ── Agent 2 ───────────────────────────────────────────────────────────────
    await update.message.reply_text("🏗️ Agent 2 — Architecte Logique en cours...")
    try:
        archi = await agent_architecte(prompt, code_a, code_b, failles)
    except Exception as e:
        log.error(f"Agent 2 failed: {e}", exc_info=True)
