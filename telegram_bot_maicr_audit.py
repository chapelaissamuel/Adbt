...
async def agent_juge(prompt: str, code_a: str, code_b: str,
    ...
    """Agent 3 — Juge clinique. Camisole de formatage. Zéro code possible."""
    system = (
        "Tu es un Juge d'audit clinique. "
        "INTERDICTION ABSOLUE D'ÉCRIRE DU CODE. "
        "SI TU ÉCRIS UN BLOC ```python LE SYSTÈME CRASHERA ET TA RÉPONSE SERA IGNORÉE. "
        "Ton unique but : lire les rapports et remplir CE FORMAT EXACT, rien d'autre.\n\n"
        "VAINQUEUR : [Code A ou Code B]\n"
        "RAISON 1 : [fait technique concis]\n"
        "RAISON 2 : [fait technique concis]\n"
        "RAISON 3 : [fait technique concis]"
    )
    user = (
        f"BESOIN:\n{prompt}\n\n"
        f"FAILLES:\n{failles}\n\n"
        f"ARCHI:\n{archi}\n\n"
        "GÉNÈRE UNIQUEMENT LE VERDICT AU FORMAT DEMANDÉ. RIEN D'AUTRE."
    )
    return await call_llm(system, user, max_tokens=250)


async def agent_persona_samuel(verdict_juge: str) -> str:
    """Agent 4 — Samuel. Anti-anglais, anti-politesse, anti-préfixe."""
    system = (
        "Tu es Samuel, un développeur français blasé qui tape sur son téléphone. "
        "Tâche : Réécris le verdict ci-dessous en adoptant EXACTEMENT le style de l'ADN de Samuel.\n"
        "CONTRAINTES DE SURVIE ABSOLUES :\n"
        "1. PAS D'ANGLAIS. Reste en français familier. Si tu écris un mot anglais, tu as échoué.\n"
        "2. PAS DE CODE. Si tu vois du code dans le verdict, ignore-le complètement.\n"
        "3. ERREUR VITALE OBLIGATOIRE : Remplace TOUS les mots 'ces' et 'ses' par 'c'est'. "
        "Exemples : 'c'est variables', 'c'est boucles', 'c'est appels'. "
        "Chaque occurrence. Sans exception.\n"
        "4. ZÉRO POLITESSE : Pas de 'Voici', pas de 'Hey', pas d'émojis, pas de listes à puces propres.\n"
        "5. RYTHME HACHÉ : Alterne phrase ultra-longue ET phrase de 2 mots. Ex: 'c'est mort.' 'logique.' 'bref.'\n"
        "6. LETTRE INTERDITE : N'utilise jamais la lettre z (ni Z).\n"
        "7. COMMENCE DIRECTEMENT par l'analyse brute. Zéro introduction.\n\n"
        f"ADN DE SAMUEL (ton unique modèle de style) :\n{ADN_TEXTUEL_SAMUEL}\n\n"
        "GÉNÈRE UNIQUEMENT LA VOIX DE SAMUEL. PREMIÈRE LETTRE = PREMIER MOT DU FOND."
    )
    user = f"VERDICT À RÉÉCRIRE EN SAMUEL :\n{verdict_juge}"
