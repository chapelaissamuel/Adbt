...
async def agent_persona_samuel(verdict_juge: str) -> str:
    """
    Agent 4 — Transcription vocale mode BRUTAL.
    Prefill long + chaos grammatical forcé pour contrer le RLHF.
    Double post-processing : suppression majuscules résiduelles + blocs code.
    """
    ...
    # PREFILL LONG — chaos installé dès le début, RLHF ne peut pas reprendre le contrôle
    prefill = "ptn c'est nimp se code a la rache"
    ...
    payload = {
        "model":       MODEL_NAME,
        "max_tokens":  600,
        "temperature": 0.92,   # plus haut = moins de RLHF
        "top_p":       0.95,
        "system":      system,
        "messages":    messages,
    }
    ...
    async with _hx.AsyncClient(timeout=API_TIMEOUT) as client:
        for attempt in range(API_RETRIES):
            try:
                ...
                # 1. Supprimer tous les blocs de code (```...``` et ` `)
                raw = _re.sub(r'```[\s\S]*?```', '', raw)
                raw = _re.sub(r'`[^`]+`', lambda m: m.group().strip('`'), raw)
                # 2. Tout en minuscules
                raw = raw.lower()
                # 3. Supprimer les points finaux de phrase (pas les x) )
                raw = _re.sub(r'\.\s', ' ', raw)
                raw = raw.rstrip('.')
                # 4. Supprimer les puces/listes
