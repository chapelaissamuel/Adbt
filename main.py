"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  MAICR-AUDIT · TELEGRAM BOT · Relecture de Code v1.3                       ║
║  SWEET SPOT ENTROPIQUE · Prefill Juge verrouillé · Destruction ponctuation ║
║  PIPELINE 4 AGENTS : Testeur de Faille → Architecte → Juge → Persona       ║
║  PERSONA ENGINE "Samuel" · temperature=0.90 · entropy_balanced             ║
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    async with httpx.AsyncClient(timeout=API_TIMEOUT) as client:
        for attempt in range(API_RETRIES):
            try:
                ...
                # ── POST-PROCESSING BRUTAL (DESTRUCTION DE PONCTUATION) ───────
                ...
                # 3. DESTRUCTION TOTALE DE LA PONCTUATION (anti-GPTZero)
                # On remplace par des espaces pour garder la séparation entre mots
                raw = raw.replace('.', ' ').replace(',', ' ').replace('!', ' ')
                raw = raw.replace('?', ' ').replace(';', ' ').replace(':', ' ')
                raw = raw.replace('...', ' ').replace('..', ' ')

                # 4. Supprimer les doubles espaces créés par le remplacement
                raw = re.sub(r'\s+', ' ', raw)

                # 5. Recoller le prefill
                result = prefill + " " + raw.strip()

                # 6. Vérification de sécurité : si "mais/donc/alors" présents, les briser
