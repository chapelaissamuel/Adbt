"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  MAICR-AUDIT · TELEGRAM BOT · Relecture de Code v1.4                       ║
║  ARCHITECTURE ATOMIQUE · Short-Burst SMS · Interdiction Subordination      ║
║  PIPELINE 4 AGENTS : Testeur de Faille → Architecte → Juge → Persona       ║
║  PERSONA ENGINE "Samuel" · temperature=0.90 · coupure 12 mots/ligne        ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
async def agent_juge(prompt: str, code_a: str, code_b: str,
    ...
    """Agent 3 — Data Starvation + Prefill forcé pour verrouiller le format."""
    system = (
        "Tu es un script d'extraction de texte. "
        "N'ÉCRIS AUCUNE LIGNE DE CODE PYTHON. "
        "Tu dois sortir EXACTEMENT ces 4 lignes, rien d'autre, pas d'explications, pas de code :\n"
        "VAINQUEUR : [A ou B]\n"
    ...
    user = (
        ...
        "Sors exactement les 4 lignes demandées, pas de code, pas d'explications :"
    )

    # PREFILL POUR FORCER LE FORMAT (anti-dérive code)
    messages = [
        {"role": "user", "content": user},
        {"role": "assistant", "content": "VAINQUEUR : "}  # Amorce verrouillée
    ]
    ...
    # Appel direct (pas via call_llm pour garder le prefill custom)
    async with httpx.AsyncClient(timeout=API_TIMEOUT) as client:
        for attempt in range(API_RETRIES):
            try:
                resp = await client.post(OR_URL, headers=headers, json=payload)
                ...
                # Vérification de sécurité : si on détecte 'def ' ou 'import ', on tronque
                if "def " in content or "import " in content or "```" in content:
                    log.warning("Agent 3 a tenté de sortir du code, application du masque")
                    lines = content.split('\n')
                    filtered = [l for l in lines if not l.strip().startswith(('def ', 'import ', 'class ', '```'))]
                    content = '\n'.join(filtered[:4])  # Max 4 lignes
                # Recoller le prefill amorcé
                return "VAINQUEUR : " + content
            except Exception as e:
                if attempt == API_RETRIES - 1:
                    return "VAINQUEUR : A\nR1 : erreur api\nR2 : erreur api\nR3 : erreur api"
...
async def agent_persona_samuel(verdict_juge: str) -> str:
    """
    Agent 4 — Transcription vocale ARCHITECTURE ATOMIQUE (Short-Bursting).
    System prompt v1.4 : interdiction subordination + structure telegram multi-SMS.
    Post-processing : sauts de ligne atomiques + coupure forcée 12 mots/ligne.
    """
    verdict_propre = _parse_verdict(verdict_juge)

    # SYSTEM PROMPT — Architecture Atomique (Short-Bursting)
    system = (
        "tu es un moteur de transcription sms. "
        "tu transcris le message d'un mec qui vibe-code sur son redmi dans le finistère.\n\n"

        "RÈGLES ABSOLUES — violation = échec critique :\n"
        "- ZÉRO majuscule. même en début de phrase.\n"
        "- ZÉRO ponctuation finale. pas de point. pas de point d'exclamation.\n"
        "- ZÉRO bloc de code. ZÉRO backtick.\n"
        "- ZÉRO emoji.\n"
        "- ZÉRO anglais.\n"
        "- ZÉRO mot 'le/la/les/ces/ses/ce/ceux' → remplace par 'c'est'.\n\n"

        "6. INTERDICTION DE SUBORDINATION (CRITIQUE) : Tu as l'interdiction absolue d'utiliser "
        "les mots 'qui', 'que', 'quoi', 'dont', 'où', 'parce que', 'vu que', 'pour que', "
        "'bien que', 'lequel', 'laquelle', 'duquel'. "
        "Tu ne dois JAMAIS relier deux idées avec ces mots. "
        "Zéro proposition subordonnée. Zéro phrase complexe.\n\n"

        "7. STRUCTURE TELEGRAM (SHORT-BURST) : Coupe tes idées par des doubles sauts de ligne (\\n\\n). "
        "Comporte-toi comme si tu envoyais plusieurs petits SMS à la suite, pas un seul long message. "
        "LONGUEUR MAXIMALE PAR LIGNE : 12 mots. Pas une de plus.\n\n"

        "8. STYLE TÉLÉGRAPHIQUE — PHRASES NOMINALES : Fais des phrases sans verbe conjugué. "
        "Utilise des fragments. Des ruptures. Des apostrophes sur les sujets.\n"
        "Exemples de format exact attendu :\n"
        "ptn c'est mort\n\n"
        "bcrypt c'est le boss pour ça\n\n"
        "sha256 lâche l'affaire face aux gpu\n\n"
        "bref le juge a géré\n\n"
        "c'est variables pas initialisées ptn\n\n"
        "syntaxe de merde\n\n"
        "abusé ce truc\n\n"

        "OBLIGATIONS :\n"
        "- au moins 4 occurrences de : ptn / mdr / dcp / genre / x) / ptdrrr\n"
        "- alternance violente des longueurs : 2 mots puis 10 mots puis 3 mots\n"
        "- parler technique sans les termes exacts, genre 'c'est fonction là qui plante'\n\n"

        f"ADN VOCAL RÉEL (imite ce registre fragmenté) :\n{ADN_TEXTUEL_SAMUEL}"
    )

    # PREFILL ATOMIQUE — initie la fragmentation immédiate
    prefill = "ptn c'est mort\n\ngenre vraiment"
    ...
    # PAYLOAD SWEET SPOT ENTROPIQUE (équilibre chaos/cohérence)
    payload = {
        "model":             MODEL_NAME,
        "max_tokens":        600,
        "temperature":       0.90,   # ↓ Réduit de 0.95 (conservation des règles)
        "top_p":             0.95,
        "frequency_penalty": 0.6,    # ↓ Réduit de 1.2 (évite l'amnésie des contraintes)
        "presence_penalty":  0.4,    # ↓ Réduit de 0.8 (garde le zigzag mais pas l'oubli)
        "system":            system,
        "messages":          messages,
    }
    headers = {
    ...
    async with httpx.AsyncClient(timeout=API_TIMEOUT) as client:
        for attempt in range(API_RETRIES):
            try:
                ...
                # ── POST-PROCESSING ARCHITECTURE ATOMIQUE ─────────────────────
                # 1. Supprimer blocs code
                ...
                # 3. Destruction des subordonnants résiduels (sécurité)
                subordonnants = [' qui ', ' que ', ' quoi ', ' dont ', ' où ',
                                 ' parce que ', ' vu que ', ' pour que ',
                                 ' bien que ', ' lequel ', ' laquelle ']
                for sub in subordonnants:
                    raw = raw.replace(sub, ' ')

                # 4. Remplacer la ponctuation classique par sauts de ligne ou espaces
                raw = raw.replace('.', '\n\n').replace('!', '\n\n').replace('?', '\n\n')
                raw = raw.replace(',', ' ').replace(';', ' ').replace(':', ' ')

                # 5. Normaliser les espaces (hors sauts de ligne) et les sauts multiples
                raw = re.sub(r'[^\S\n]+', ' ', raw)    # espaces non-saut → espace simple
                raw = re.sub(r'\n+', '\n\n', raw)       # sauts multiples → \n\n

                # 6. Coupure forcée si une ligne dépasse 12 mots
                lines = raw.split('\n\n')
                new_lines = []
                for line in lines:
                    words = line.split()
                ...
                # 7. Recoller le prefill
                result = prefill + "\n\n" + raw.strip()

                # 8. Nettoyage final des connecteurs logiques résiduels
                result = result.replace(' mais ', ' ').replace(' donc ', ' ')
                result = result.replace(' alors ', ' ').replace(' si ', ' ')

                return result

            except Exception as e:
                if attempt == API_RETRIES - 1:
                    return f"ptn c'est mort\n\nl'api répond plus\n\nbref ({e})"
                await asyncio.sleep(2 ** attempt)

    return "ptn c'est mort\n\néchec total retries\n\nbref"
